...
VIEW.produtos = async ()=>{
  ...
  $('#view').innerHTML=`
   <div class="view">
    ...
    <div class="grid">
      <div class="panel col-12" id="painelCustoVenda">
        <div class="panel-head"><div><h3>Catálogo · Custo, Venda e Margem</h3><p>Carregando...</p></div></div>
      </div>
    </div>
   </div>`;
  ...
  // carrega o catálogo de custo/venda do Supabase (tabela produtos_venda)
  try{
    PRODUTOS_VENDA = await BadareDB.loadProdutosVenda();
    renderPainelCustoVenda();
  }catch(err){
    $('#painelCustoVenda').innerHTML = `<div class="panel-head"><div><h3>Catálogo · Custo, Venda e Margem</h3><p style="color:var(--danger)">Erro ao carregar: ${esc(err.message||err)}</p></div></div>`;
  }
};
function renderPainelCustoVenda(){
  const el = $('#painelCustoVenda'); if(!el) return;
  const rows = [...PRODUTOS_VENDA].sort((a,b)=>(b.venda||0)-(a.venda||0));
  el.innerHTML = `
    <div class="panel-head"><div><h3>Catálogo · Custo, Venda e Margem</h3><p>${rows.length} produto(s) · vindo da planilha de vendas</p></div></div>
    <div class="tbl-wrap"><table>
      <thead><tr><th>Marca</th><th>Produto</th><th style="text-align:right">Custo</th><th style="text-align:right">Venda</th><th style="text-align:right">Margem</th><th style="text-align:right">Und. (jan-ago)</th></tr></thead>
      <tbody>${rows.map(p=>{
        const margem = (p.custo && p.venda) ? ((p.venda-p.custo)/p.venda*100) : null;
        return `<tr style="cursor:default">
          <td class="muted">${esc(p.marca||'—')}</td>
          <td class="cell-strong">${esc(p.produto)}</td>
          <td style="text-align:right">${p.custo!=null?fmtBR(p.custo):'—'}</td>
          <td style="text-align:right">${p.venda!=null?fmtBR(p.venda):'—'}</td>
          <td style="text-align:right">${margem!=null?margem.toFixed(0)+'%':'—'}</td>
          <td style="text-align:right" class="muted">${fmtN(p.unidades_periodo||0)}</td>
        </tr>`;
      }).join('') || '<tr><td colspan="6"><div class="empty">Nenhum produto cadastrado</div></td></tr>'}</tbody>
    </table></div>`;
}
